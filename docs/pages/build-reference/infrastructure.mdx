...
export const {
  ...
  resources: { ios, android },
} = resourceSpecs.data;

export function HardwareList({ platform }) {
  const data = platform === 'ios' ? ios : android;
  return (
    <ul>
      {values(data).map(({ hardware }) =>
        keys(hardware).map(name => {
          const { cpu, memory, description } = hardwareSpecs[name];
          return (
            <li key={name}>
